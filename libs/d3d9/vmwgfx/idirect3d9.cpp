...
export IDirect3DDevice9* IDirect3D9_vmwgfx::createDevice(
    ...
    DLOG("IDirect3D9_vmwgfx::createDevice(\n" <<
        "    fd:" << Adapter << ",\n" <<
    ...
    if (drmIoctl(Adapter, DRM_IOCTL_MODE_CREATE_DUMB, &creq) < 0) {
        std::cerr << "\033[1;31m" //RED BOLD
            << "[IDirect3D9_vmwgfx] ODX ERROR: Failed to create framebuffer:" << strerror(errno) << std::endl
            << "[IDirect3D9_vmwgfx] IDirect3DC9::CreateDevice fails and returns NULL.\033[0;0m" << std::endl
            << std::endl;
    ...
    //set buffer as master
    drmSetMaster(Adapter);

    //test: save framebuffer to a file
...
extern "C" __attribute__((visibility("default"))) IDirect3DDevice9* odx_create_vmwgfx_device(
    IDirect3D9* d3d9, UINT Adapter, D3DDEVTYPE DeviceType,
    HWND hFocusWindow, DWORD BehaviorFlags,
    D3DPRESENT_PARAMETERS* pPresentationParameters
) {
    return IDirect3D9_vmwgfx::createDevice(d3d9, Adapter, DeviceType, hFocusWindow, BehaviorFlags, pPresentationParameters);
}


std::shared_ptr<std::function<HRESULT(REFIID, void**)>> IDirect3D9_vmwgfx::createQueryInterface(IDirect3DDevice9* device) {
...
std::shared_ptr<std::function<ULONG()>> IDirect3D9_vmwgfx::createAddRef(IDirect3DDevice9* device) {
...
std::shared_ptr<std::function<ULONG()>> IDirect3D9_vmwgfx::createRelease(IDirect3DDevice9* device) {
    std::shared_ptr<std::function<ULONG()>> fun = std::make_shared<std::function<ULONG()>>([device]() -> ULONG {
        return 0;
    });
